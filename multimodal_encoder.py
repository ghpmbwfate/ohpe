...
# Force offline mode - never try to download from HuggingFace
...
def _load_clip_local(clip_model_obj, cache_dir):
    ...
    # Try HuggingFace cache locations for openai CLIP ViT-B-32
    search_dirs = [
    ...
    # Try direct file in clip subdirectory
    direct_path = os.path.join(cache_dir, "clip", "ViT-B-32.pt")
    ...
    # Try legacy direct file
    direct_path2 = os.path.join(cache_dir, "open_clip_pytorch_model.bin")
    if os.path.exists(direct_path2):
        state_dict = torch.load(direct_path2, map_location="cpu", weights_only=True)
        clip_model_obj.load_state_dict(state_dict, strict=False)
        print(f"[MultimodalEncoder] Loaded CLIP weights from {direct_path2}")
        return True
...
def _get_tokenizer_local(cache_dir):
    """Get CLIP tokenizer from local BPE file."""
    # Try clip subdirectory first
    bpe_path = os.path.join(cache_dir, "clip", "bpe_simple_vocab_16e6.txt.gz")
    if os.path.exists(bpe_path):
        tokenizer = open_clip.tokenizer.SimpleTokenizer(bpe_path)
        print(f"[MultimodalEncoder] Loaded tokenizer from {bpe_path}")
        return tokenizer

    # Fall back to open_clip's default (will try to download)
    print("[MultimodalEncoder] Warning: No local BPE vocab, using default tokenizer (may download)")
...
class MultimodalConditionEncoder(nn.Module):
    ...
    def __init__(self, cond_dim=512, swin_model='swin_base_patch4_window7_224',
        ...
        self.clip_tokenizer = _get_tokenizer_local(cache_dir)
